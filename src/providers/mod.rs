...
pub fn model_for_role<'a>(role: &str, config: &'a Config) -> Result<&'a str> {
    match role {
        ...
        // code-review workflow roles — fall back to qa model (14b)
        "reviewer"    => Ok(&config.models.qa),
        "security"    => Ok(&config.models.qa),
        "performance" => Ok(&config.models.qa),
        "reporter"    => Ok(&config.models.qa),
